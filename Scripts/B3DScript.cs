		...
		if (Ccase == 111)
		{
			...
			for (;;)
			{
				...
				if (Ccase == 222)
				{
					...
					{
						go.GetComponent<RoomBlock>().Init();
					}			
				...
				else if (Ccase == 333)
				{
					...
					newObject.AddComponent<BlockType>().Type = type;
					newObject.GetComponent<BlockType>().MatNum = 0;
					...
					else if (type == 4)
					{
						...
						if (SpcOb != null)
						{
							PosNmatr Spc = SpcOb.GetComponent<PosNmatr>();
					...
					else if (type == 5)
					{
						...
						newObject.AddComponent<JoinBlock>().name = System.Text.Encoding.UTF8.GetString(JoinName).Trim(new char[]{'\0'});
					...
					else if (type == 8)
					{
						...
						newObject.GetComponent<BlockType>().MatNum = TexInts[matNum[matNum.Count-1]];
					...
					{
						pos+=16;  
						newObject.AddComponent<Block09>().xz4eEto = new Vector4(System.BitConverter.ToSingle(resource,pos),System.BitConverter.ToSingle(resource,pos+8),System.BitConverter.ToSingle(resource,pos+4),System.BitConverter.ToSingle(resource,pos+12));
						pos+=16;  
					...
					{
						var a = newObject.AddComponent<LODManager>();
					...
					{
						newObject.AddComponent<RoomBlock>().room = newObject;
					...
					{
						PosNmatr p =  newObject.AddComponent<PosNmatr>();
						byte[] bt = new byte[12];
						
						System.Array.Copy(resource,pos,bt,0,12); pos+=12;
						//p.matrix = new [3][];
						p.matrix[0] = new Vector3(System.BitConverter.ToSingle(bt,0),System.BitConverter.ToSingle(bt,4),System.BitConverter.ToSingle(bt,8));

						System.Array.Copy(resource,pos,bt,0,12); pos+=12;
						p.matrix[1] = new Vector3(System.BitConverter.ToSingle(bt,0),System.BitConverter.ToSingle(bt,4),System.BitConverter.ToSingle(bt,8));
						System.Array.Copy(resource,pos,bt,0,12); pos+=12;
						p.matrix[2] = new Vector3(System.BitConverter.ToSingle(bt,0),System.BitConverter.ToSingle(bt,4),System.BitConverter.ToSingle(bt,8));
						System.Array.Copy(resource,pos,bt,0,12); pos+=12;
						p.position = new Vector3(System.BitConverter.ToSingle(bt,0),System.BitConverter.ToSingle(bt,8),System.BitConverter.ToSingle(bt,4));
						pos+=8; 
					}
					else if (type == 25)
					{
					...
					else if (type == 28)
					{
						...
						newObject.AddComponent<rotationSprite>().camer = rootObj.transform.parent.GetComponent<GameManager>().camera;
					...
					else if (type == 35)
					{
						...
						pos+=4;//textureNum?
						newObject.GetComponent<BlockType>().MatNum = matNum;
						int j_null, i_null2;
							System.Array.Copy(resource,pos,buff,0,4); 	pos+=4;
						j_null = System.BitConverter.ToInt32(buff,0);
						//
			...
			foreach (var go in Blocks05)
			{
				...
				string objName = go.GetComponent<JoinBlock>().name;
				if (objName != "")
				{
					...
					go.GetComponent<JoinBlock>().obj = obj;
...
class LODManager : MonoBehaviour
{
